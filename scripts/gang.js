...
export async function main(ns) {
  ...
  while (true) {
    ...
    let minCombatName = ""
    for (let member of members) {
      ns.gang.setMemberTask(member, warfare && (Math.random() < 2/3) ? "Territory Warfare" : pickRandom(ns.args))
      ...
      if ((memberObj.str < minCombat || !minCombatName) && memberObj.name !== minHackingName) {
    ...
    function pickRandomUnallocated() {
      ...
      const choice = unallocated[Math.floor(Math.random() * unallocated.length)];
    ...
    if (warfare) {
      ...
      ns.gang.setMemberTask(members[Math.floor(Math.random() * members.length)], pickRandom(ns.args))
    } else {
    ...
    if (warfare) {
      ns.gang.setMemberTask(members[pickRandomUnallocated()], "Territory Warfare");    
    }
