...
export async function main(ns) {
  ...
  for (let faction of allFactions) {
    ...
    matrix.push([faction, ...matrix[0].slice(1).map(i => Number(augs.includes(i)))])
    ...
    matrix.forEach((_, row) => {
      if (row === 0) {
        ...
        matrix[row] = [...matrix[row], ...Array(remainder.length).fill(Number(row === matrix.length - 1))]
